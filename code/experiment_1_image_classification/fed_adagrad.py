...
def federated_learning(i_w, data_client, C, P, R, E, learning_rate, b_size, epsilon=1e-8):
    
    global total_clients_list, participating_client_list, G
    
    global_model.load_state_dict(i_w)
    
    # Initialize accumulated squared gradient for Adagrad
    G = None
    ...
    for r in range(1, R+1):
        ...
        # Initialize the sum of squares for Adagrad if not done
        if G is None:
            G = {key: torch.zeros_like(param) for key, param in update_avg.items()}

        # Adagrad update rule for each parameter
        for key in i_w:
            # Accumulate the sum of squares of gradients
            G[key] += update_avg[key] ** 2

            # Update weights using Adagrad rule
            i_w[key] -= learning_rate * update_avg[key] / (torch.sqrt(G[key]) + epsilon)
