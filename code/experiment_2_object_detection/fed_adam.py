...
def training(i_w, E, r, c):
    ...
    local_model.train(data=f"/data/ex2/{fl_a}/{set_up}/c{c}.yaml", 
                      ...
                      val=True, device=1, warmup_epochs=0)
...
def federated_learning(i_w, C, P, R, E, b_size, lr=0.001, tau=1e-3, beta_1=0.9, beta_2=0.999):
    global global_model, m, v, t
    ...
    for r in range(1, R + 1):
        ...
        if m is None:
            m = {key: torch.zeros_like(param) for key, param in average_gradients.items()}
            v = {key: torch.zeros_like(param) for key, param in average_gradients.items()}

        # Adam update rule for each parameter
        t += 1
        for key in i_w:
            # Update biased first moment estimate
            m[key] = beta_1 * m[key] + (1 - beta_1) * average_gradients[key]
            # Update biased second raw moment estimate
            v[key] = beta_2 * v[key] + (1 - beta_2) * (average_gradients[key] ** 2)
            ...
            i_w[key] -= lr * m_hat / (torch.sqrt(v_hat) + tau)
        ...
        # Perform validation
        validation_results = val_model.val(data=f"/data/ex2/{fl_a}/c5.yaml", project=f"{dst_folder}/val/round_{r}", imgsz=512, batch=4, split='val', workers=0, device=1)
...
dst_folder = f"{fl_a}_{forname}_Fed_adam_{learning_rate}_{opti}"
...
validation_results = l_model.val(data=f"/data/ex2/{fl_a}/c5.yaml", project=f"{dst_folder}/val/round_0", imgsz=512, batch=4,split='val', workers=0,device=1)
validation_dict["round_0"] = validation_results
print(validation_results)

#=================================================================client_1====================
l_model = YOLO("/data/ex2/initial_weights.pt").to(device)
l_model.val(data=f"/data/ex2/{fl_a}/{set_up}/c1.yaml", project=f"{dst_folder}/train/round_0_client_1", imgsz=512, batch=4, split='train',  workers=0,device=1)

#=================================================================client_2====================
l_model = YOLO("/data/ex2/initial_weights.pt").to(device)
l_model.val(data=f"/data/ex2/{fl_a}/{set_up}/c2.yaml", project=f"{dst_folder}/train/round_0_client_2", imgsz=512, batch=4, split='train',  workers=0,device=1)


#=================================================================client_3====================
l_model = YOLO("/data/ex2/initial_weights.pt").to(device)
l_model.val(data=f"/data/ex2/{fl_a}/{set_up}/c3.yaml", project=f"{dst_folder}/train/round_0_client_3", imgsz=512, batch=4, split='train',  workers=0,device=1)


#=================================================================client_4====================
l_model = YOLO("/data/ex2/initial_weights.pt").to(device)
l_model.val(data=f"/data/ex2/{fl_a}/{set_up}/c4.yaml", project=f"{dst_folder}/train/round_0_client_4", imgsz=512, batch=4, split='train',  workers=0,device=1)
# clear_output(wait=False)
